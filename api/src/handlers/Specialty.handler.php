...
class SpecialtyHandler extends MainHandler{
    ...
    public function getStudentSpecialtiesHandler(Request $request, Response $response) {
        ...
        if(!$student) throw new ArgumentException('Student not found');
        
        $specialties = $student->specialties()->get();
        if(!$specialties) throw new ArgumentException('Invalid student id');
    ...
    public function getProfileSpecialtiesHandler(Request $request, Response $response) {
        ...
        //if(!$specialties) throw new ArgumentException('Invalid profile id');
    ...
    public function createSpecialtyHandler(Request $request, Response $response) {
        ...
        foreach($data['badges'] as $bslug)
        {
            ...
            else throw new ArgumentException('Invalid badge: '.$bslug);
        }
    ...
    public function updateSpecialtyHandler(Request $request, Response $response) {
        ...
        if(!$specialty) throw new ArgumentException('Invalid specialty slug or id: '.$specialtyId);
        ...
        {
            if(count($data['badges'])<2) throw new ArgumentException('A specialty must have at least two badges');
            $badges = [];
            foreach($data['badges'] as $bslug)
            {
                $badge = Badge::where('slug', $bslug)->first();
    ...
    public function deleteSpecialtyHandler(Request $request, Response $response) {
        ...
        if(count($badges)>2) throw new ArgumentException('Only specialties with 2 or less badges can be removed, please remove badges from this specialty first');
    ...
    public function addSpecialtiesToProfileHandler(Request $request, Response $response) {
        ...
        if(empty($specialtiesObj['specialties'])) throw new ArgumentException('There was an error retrieving the badges');
        
        $profile = Profile::find($profileId);
        if(!$profile) throw new ArgumentException('Invalid specialty id: '.$profileId);
    ...
    public function deleteSpecialtiesFromProfileHandler(Request $request, Response $response) {
        ...
        $specialtiesObj = $request->getParsedBody();
        if(empty($specialtiesObj['specialties'])) throw new ArgumentException('There was an error retrieving the specialties');
        foreach($specialtiesObj['specialties'] as $specialtyId)
        {
            $specialty = Specialty::find($specialtyId);
            if(!$specialty) throw new ArgumentException('There is no specialty with ID '.$specialtyId);
        }
        ...
        else throw new ArgumentException('The specialties array is empty');
